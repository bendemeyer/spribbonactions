...
function checkForAndLoadjQuery() {
    if (document.readyState === "complete") {
        if (typeof jQuery == 'undefined') {
            ...
            jQueryScriptLink.src = "http://code.jquery.com/jquery-latest.js";
...
function youtubeLinkReplace() {
    ...
    //var objectTag = '<object width="100%" height="100%"><param name="movie" value="{PROTOCOL}//www.youtube.com/v/{VIDEO_ID}?fs=1&enablejsapi=1&rel=0" /><param name="allowFullScreen" value="true" /><param name="allowScriptAccess" value="always" /><embed id="ytplayer" src="{PROTOCOL}//www.youtube.com/v/{VIDEO_ID}?fs=1&enablejsapi=1&rel=0" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="100%" height="100%"></embed></object>';
    //var embedTagOnly = '<embed id="ytplayer" src="{PROTOCOL}//www.youtube.com/v/{VIDEO_ID}?fs=1&enablejsapi=1&rel=0" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="100%" height="100%"></embed>';
    ...
    videoContainers.each(function () {
        ...
        if (videoUrl.length) {
            ...
            //videoTag = browserIsIE ? embedTagOnly.replace(/\{VIDEO_ID\}/g, id).replace(/\{PROTOCOL\}/g, window.location.protocol) : objectTag.replace(/\{VIDEO_ID\}/g, id).replace(/\{PROTOCOL\}/g, window.location.protocol);
...
function replaceAudioLinkWithEmbed() {
    ...
    $("a.audioLink").parent("p").each(function () {
        ...
        if (getInternetExplorerVersion() != -1 && getInternetExplorerVersion() < 9) {
            fullObjectHTML = '<embed id="audioPlayer_' + audioEmbedCount + '" src="/_layouts/BeenJammin.SharePoint.CustomRibbonActions/player.swf" Flashvars="soundFile=' + audioURL + audioTitleURL + audioArtistURL + '&animation=no&playerID=audioplayer_' + audioEmbedCount + '" type="application/x-shockwave-flash" allowscriptaccess="always" width="100%" height="24"></embed>';
        }
            ...
            var paramFlashvars = '<param name="flashvars" value="soundFile=' + audioURL + audioTitleURL + audioArtistURL + '&animation=no&playerID=audioplayer_' + audioEmbedCount + '" />';
